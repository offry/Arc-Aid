...
class loader():
    def __init__(self, root, is_train=True, data_len=None):
        ...
        train_val_file = open(os.path.join(self.root, 'split.txt'))
        img_name_list = []
        ...
        if not self.is_train:
            ...
            #     im = imageio.imread(os.path.join(self.root, 'images', test_file))
            #     self.test_img.append(im)
            # self.test_img = [imageio.imread(os.path.join(self.root, 'images', test_file)) for test_file in
            #                  test_file_list[:data_len]]
            self.test_img = []
            for test_file in test_file_list[:3]:
