        ...
        for phase in ['train', 'test']:
            ...
            for draw_data, image_data in dataloaders:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # periods_outputs = periods_embeddings(inputs)
...
def train_shapes_with_periods_comparison(arch_type, device, train_dataloaders, test_dataloaders, exp,
    ...
    if "coinnet" in arch_type:
        if "pretrained" in arch_type:
            exp_number = "experiment_" + experiment_name.split("_")[3]
            arch_dir = "pretrained_coinnet_"
        else:
            arch_dir = "coinnet_"
    elif "glyphnet" in arch_type:
        arch_dir = "glyphnet_"
    elif 'efficient' in arch_type:
        arch_dir = 'efficientnet_'
    ...
    if "glyphnet" in arch_type:
        ...
        for param in periods_model.conv_block.parameters():
            param.requires_grad = True
        if args.sub_periods:
            periods_model.fc = nn.Linear(2208, 5).to(device)
        else:
            periods_model.fc = nn.Linear(2208, 3).to(device)
    else:
        for param in periods_model.parameters():
            param.requires_grad = True
        if args.sub_periods:
            periods_model.classifier[1] = nn.Linear(in_features=1536, out_features=5).to(device)
        else:
            periods_model.classifier[1] = nn.Linear(in_features=1536, out_features=3).to(device)
